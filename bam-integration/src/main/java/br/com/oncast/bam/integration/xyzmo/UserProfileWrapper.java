...
public class UserProfileWrapper {
	...
	private BiometricServerFactory biometricServerFactory;

	public UserProfileWrapper(BiometricServerFactory biometricServerFactory) {
		this.biometricServerFactory = biometricServerFactory;
	}
	...
	public void addUser(String userId, String fullName) throws UserProfileException {
		try {
			...
			User_Add_v1Response userAddResponse = biometricServerFactory
					.getWebServiceProxyUserAndProfile().user_Add_v1(bioUser);
			if (userAddResponse.getUser_Add_v1Result().getBaseResult() != WebServiceUserAndProfileStub.BaseResultEnum.ok) {
				throw new UserProfileException(userAddResponse.getUser_Add_v1Result()
						.getErrorInfo().getErrorMsg());
			}
	...
	public void deleteUser(String userId) throws UserProfileException {
		try {
			...
			bioUser.setBioUserId(userId);
			User_Delete_v1Response userDeleteResponse = biometricServerFactory
					.getWebServiceProxyUserAndProfile().user_Delete_v1(bioUser);
			if (userDeleteResponse.getUser_Delete_v1Result().getBaseResult() != WebServiceUserAndProfileStub.BaseResultEnum.ok) {
				throw new UserProfileException(userDeleteResponse.getUser_Delete_v1Result()
						.getErrorInfo().getErrorMsg());
			}
		} catch (RemoteException e) {
			throw new UserProfileException(e);
